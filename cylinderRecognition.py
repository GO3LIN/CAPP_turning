...
class cylinderRecognition:
	...
	def process(self, closed_s, stock):
		...
		if duplicated_cs:
			...
			related_advanced_faces = []
			for cs in duplicated_cs:
				if((cs.radius*2)>=stock.sd):
					continue
				afs = closed_s.getAdvancedFacesFromSurfaceType(cs)
				for af in afs:
					related_advanced_faces.append(af)
				#related_advanced_faces.append(closed_s.getAdvancedFacesFromSurfaceType(cs))
			raf_temp = list(related_advanced_faces)

			while(len(raf_temp)>0):
				first = raf_temp.pop()
				first_lines = first.getEdgeType(stepCode.line)
				first_lines_step = r_tools.getStepFromObjects(first_lines)
				for raf in related_advanced_faces:
					if(first.stepLine != raf.stepLine):
						raf_lines = raf.getEdgeType(stepCode.line)
						raf_lines_step = r_tools.getStepFromObjects(raf_lines)
						if(set(raf_lines_step) == set(first_lines_step)):
							raf_temp.pop(raf_temp.index(raf))
							self.foundFeatures.append([raf.stepLine, first.stepLine])

						
			'''
			for raf in related_advanced_faces:
				print raf.stepLine
			related_advanced_faces = closed_s.getAdvancedFacesFromStepSurfaceType(stepCode.cylindrical_surface)


			for i,raf in enumerate(related_advanced_faces):
				for j,raf2 in enumerate(related_advanced_faces):
					if(i!=j):
						raf_lines = raf.getEdgeType(stepCode.line)
			'''
